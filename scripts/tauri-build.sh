...
generate_latest_json() {
    ...
    # Look for AppImage with signature (preferred for updater)
    local appimage_file=""
    local appimage_sig=""

    for file in "$artifacts_dir"/*.AppImage.tar.gz; do
        if [ -f "$file" ] && [ -f "${file}.sig" ]; then
            appimage_file="$file"
            appimage_sig="${file}.sig"
            break
        fi
    done

    # Fallback to regular AppImage
    ...
    if [ -n "$appimage_file" ] && [ -n "$appimage_sig" ]; then
        local filename=$(basename "$appimage_file")
        local signature=$(cat "$appimage_sig")
        local download_url="${base_url}/${filename}"

        # Add platform entry
        platforms_json=$(cat <<EOF
    "linux-${normalized_arch}": {
      "signature": "$signature",
      "url": "$download_url"
    },
    "linux-${normalized_arch}-appimage": {
      "signature": "$signature",
      "url": "$download_url"
    }
EOF
